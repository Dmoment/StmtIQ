# frozen_string_literal: true

class WorkflowPolicy < ApplicationPolicy
  def index?
    can?(:view_invoices) # Anyone who can view data can see workflows
  end

  def show?
    record_in_workspace? && can?(:view_invoices)
  end

  def create?
    can?(:edit_transactions) # Needs edit permissions to create workflows
  end

  def update?
    record_in_workspace? && can?(:edit_transactions)
  end

  def destroy?
    record_in_workspace? && can?(:delete_data)
  end

  # Custom action for executing workflows
  def execute?
    record_in_workspace? && can?(:edit_transactions)
  end

  class Scope < Scope
    def resolve
      return scope.none unless user && current_workspace

      scope.where(workspace: current_workspace)
    end
  end
end
